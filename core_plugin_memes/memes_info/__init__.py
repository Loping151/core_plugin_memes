...
@sv_info.on_prefix(
    _INFO_KEYS,
    block=True,
    to_ai="""查询单个**已知关键词**的表情详情（关键词 / 图片需求 / 文字需求 /
可选参数 / 示例图）。

何时调用：
- 用户问「XX 表情详情 / XX 怎么用 / XX 要传几张图」
- 你打算调 `表情包制作`，但需要先确认该 meme 的图片数 / 文字数 / 可选 `--option`

⚠️ 你**必须传一个具体表情关键词**（如「拍 / 摸 / 点赞」）。如果用户只
说了某个**题材**（比如「明朝 / 老板 / 加油」）你**还没确定具体关键词**，
请改用 `表情搜索` 工具——本工具只接受 meme 名，不是题材关键字。
