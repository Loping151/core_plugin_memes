...
@sv_make.on_prefix(
    "",
    block=True,
    to_ai="""制作表情包：根据**精确关键词** + 可选图片/文字/at 生成具体的表情图。

⚠️ **本工具只接受具体表情关键词**（如「拍」「摸」「点赞」「比心」等已存在
的 meme name）。如果你只知道用户想要"某个主题"但**没有精确 keyword**，
**不要瞎猜关键词**，**先调 `表情搜索` 工具**按题材搜出候选关键词，再回来
调本工具。

何时调用：
- 用户明确给了关键词，比如「生成一张『摸』表情 / 帮我整张『拍』」
- 你刚通过 `表情搜索` / `查看表情详情` 拿到了精确关键词
- 你在聊天里需要贴一张应景表情图（先确认该关键词在表情库存在）
...
被全局禁用的表情不要调；KB 里没出现的 meme 大概率不可用。
